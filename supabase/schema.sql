...
-- In Authentication > Providers, enable Anonymous Sign-Ins before publishing.
...
-- A database trigger enforces the send interval even when someone bypasses the UI.
...
begin
  ...
  -- Serialize submissions by the same guest, including parallel requests.
...
-- Enables Realtime INSERT delivery. This statement is intended to run once.
alter publication supabase_realtime add table public.wishes;

-- Moderation: in SQL Editor, use UPDATE public.wishes SET status = 'hidden' WHERE id = '<uuid>';
-- Hidden wishes are excluded by RLS from new reads. Connected clients recheck periodically.
